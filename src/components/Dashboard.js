...
const Dashboard = () => {
    ...
    const fetchDashboardStats = async () => {
        try {
            ...
            // Calculate trends
            const trends = calculateTrends(customersData, leadsData, contactsData);

            // Calculate conversion rate
            const totalLeadsAndCustomers = customersCount + leadsCount;
            const conversionRate = totalLeadsAndCustomers > 0
                ? Math.round((customersCount / totalLeadsAndCustomers) * 100)
                : 0;
            ...
            setStats({
                ...
                contactsTrend: trends.contactsTrend,
                conversionTrend: trends.conversionTrend
            });

            // Cache data for period toggle
            setCustomersDataCache(customersData);
            setLeadsDataCache(leadsData);

            // Process monthly performance data
            generateMonthlyPerformanceData(customersData, leadsData, chartPeriod);

            // Process lead status overview data
            generateLeadStatusData(leadsData, customersCount, chartPeriod);

            // Process revenue trend data
            generateRevenueTrendData(customersData, chartPeriod);
